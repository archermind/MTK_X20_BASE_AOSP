...
/*********************start ---internal API**************************************/
static S32 pwrap_read(U32 adr, U32 *rdata);
static S32 pwrap_write(U32 adr, U32  wdata);
static S32 pwrap_wacs2(U32  write, U32  adr, U32  wdata, U32 *rdata);
...
static int _pwrap_timeout_ns (U64 start_time_ns, U64 timeout_time_ns);
...
static int _pwrap_timeout_ns (unsigned long long start_time, unsigned long long elapse_time)
{
    unsigned long long cur_time = xTaskGetTickCount() * (1000 / portTICK_RATE_MS) * 1000;
...
static inline U32 wait_for_scp_fsm_idle(U32 x)
{
    return (GET_WACS_P2P_FSM( x ) != WACS_FSM_IDLE );
}
static inline U32 wait_for_scp_fsm_vldclr(U32 x)
{
    return (GET_WACS_P2P_FSM( x ) != WACS_FSM_WFVLDCLR);
}
...
static inline U32  wait_for_scp_state_idle(loop_condition_fp fp,U32 timeout_us,void * wacs_register,void * wacs_vldclr_register,U32 *read_reg)
{
...
static inline U32 wait_for_scp_state_ready(loop_condition_fp fp,U32 timeout_us,void * wacs_register,U32 *read_reg)
{
...
/******************************************************
 * Function : pwrap_wacs_p2p()
 * Description : For scp env user
 * Parameter :
 * Return :
 ******************************************************/
static S32 pwrap_wacs_p2p( U32  write, U32  adr, U32  wdata, U32 *rdata )
{
    U32 reg_rdata=0;
    U32 wacs_write=0;
    U32 wacs_adr=0;
    ...
    return_value = wait_for_scp_state_idle(wait_for_scp_fsm_idle,TIMEOUT_WAIT_IDLE,PMIC_WRAP_WACS_P2P_RDATA,PMIC_WRAP_P2P_WACS_P2P_VLDCLR,0);
    if( return_value != 0 ) {
    ...
    WRAP_WR32(PMIC_WRAP_WACS_P2P_CMD,wacs_cmd);
    ...
    if( write == 0 ) {
        ...
        return_value = wait_for_scp_state_ready(wait_for_scp_fsm_vldclr,TIMEOUT_READ,PMIC_WRAP_WACS_P2P_RDATA,&reg_rdata);
        if( return_value != 0 ) {
		...
		*rdata = GET_WACS_P2P_RDATA( reg_rdata );
        WRAP_WR32(PMIC_WRAP_P2P_WACS_P2P_VLDCLR , 1);
    }
...
S32 pwrap_scp_read( U32  adr, U32 *rdata )
{
    return pwrap_wacs_p2p(0,adr,0,rdata);
}

S32 pwrap_scp_write( U32  adr, U32  wdata )
{
    return pwrap_wacs_p2p(1,adr,wdata,0);
}
...
static S32 _pwrap_init_sistrobe( void )
{
    ...
    for( ind=0; ind<24; ind++)
    {
        ...
        if( rdata == MT6351_DEFAULT_VALUE_READ_TEST ) {
            PWRAPLOG("_pwrap_init_sistrobe [Read Test of %s] pass,index=%d rdata=%x\n\r",PMIC_CHIP_VERSION, ind,rdata);
            result[0] |= (0x1 << ind);
		} else {
            PWRAPLOG("_pwrap_init_sistrobe [Read Test of %s] tuning,index=%d rdata=%x\n\r",PMIC_CHIP_VERSION, ind,rdata);
		}
#endif
     }
...
static U32 pwrap_scp_read_test(void)
{
    U32 rdata=0;
    U32 return_value=0;
	/* Read Test */
#ifdef SLV_6351
    return_value=pwrap_scp_read(MT6351_DEW_READ_TEST,&rdata);
    if( rdata != MT6351_DEFAULT_VALUE_READ_TEST ) {
        PWRAPREG("SCP: Read Test fail,rdata=0x%x, exp=0x5aa5,return_value=0x%x\n\r", rdata,return_value);
        return E_PWR_READ_TEST_FAIL;
    } else {
        PWRAPREG("SCP: Read MT6351 Test pass,return_value=%d\n\r",return_value);
    }
#endif

    return 0;
}
static U32 pwrap_scp_write_test(void)
{
    U32 rdata=0;
    U32 sub_return=0;
    U32 sub_return1=0;
    ...
    /* Write test using WACS2 */
#ifdef SLV_6351
    sub_return = pwrap_scp_write(MT6351_DEW_WRITE_TEST, MT6351_WRITE_TEST_VALUE);
    PWRAPREG("SCP: after MT6351 pwrap_write\n\r");
    sub_return1 = pwrap_scp_read(MT6351_DEW_WRITE_TEST,&rdata);
    if(( rdata != MT6351_WRITE_TEST_VALUE )||( sub_return != 0 )||( sub_return1 != 0 )) {
        PWRAPREG("SCP: write test error,rdata=0x%x,exp=0xa55a,sub_return=0x%x,sub_return1=0x%x\n\r", rdata,sub_return,sub_return1);
        return E_PWR_INIT_WRITE_TEST;
    } else {
        PWRAPREG("SCP: write MT6351 Test pass\n\r");
    }
#endif
...
S32 pwrap_init ( void )
{
  ...
  U32 rdata=0x0;
   ...
   PWRAPLOG("SCP pwrap_init start!!!!!!!!!!!!! \n\r");
    ...
    WRAP_WR32(PMIC_WRAP_HIPRIO_ARB_EN,WACS2);
	PWRAPLOG(" PMIC_WRAP_HIPRIO_ARB_EN=0x%x\n\r",WRAP_RD32(PMIC_WRAP_HIPRIO_ARB_EN));
	...
	/* fixme ,need clk api to set clk*/
	rdata = WRAP_RD32(0x400A4030);
	rdata |= 0x40; //open CG:bit[6]
	WRAP_WR32(0x400A4030,rdata);

	do {
		WRAP_WR32(PMIC_WRAP_WACS_P2P_EN, 0x1);
    }
	while((WRAP_RD32(PMIC_WRAP_WACS_P2P_EN) & 0x1) != 1);
    PWRAPLOG(" PMIC_WRAP_WACS_P2P_EN=%x\n\r",WRAP_RD32(PMIC_WRAP_WACS_P2P_EN));

    WRAP_WR32(PMIC_WRAP_INIT_DONE_P2P, 0x1);
	PWRAPLOG(" PMIC_WRAP_INIT_DONE_P2P=%x\n\r",WRAP_RD32(PMIC_WRAP_INIT_DONE_P2P));


	PWRAPLOG("SCP pwrap_init Done!!!!!!!!! \n\r");

   /* For pmic wrap UT */
    pwrap_ut(1);
    pwrap_ut(2);
	PWRAPLOG("AP channel pass \n\r ");
    pwrap_ut(3);
    pwrap_ut(4);
	PWRAPLOG("SCP channel pass \n\r ");
  return 0;
}
...
{
	int ret=0;
	ret = WRAP_RD32(PMIC_WRAP_INIT_DONE3);
	PWRAPLOG("PMIC_WRAP_INIT_DONE3=%x \n\r", ret);

	ret = WRAP_RD32(PMIC_WRAP_INIT_DONE_P2P);
	PWRAPLOG("is_pwrap_p2p_init_done %d \n\r",ret);
	if((ret&0x1) ==1)
		return 0;
	return -1;
}

S32 pwrap_init_scp ( void )
{
    U32 pwrap_ret=0,i=0;
    PWRAPFUC();
