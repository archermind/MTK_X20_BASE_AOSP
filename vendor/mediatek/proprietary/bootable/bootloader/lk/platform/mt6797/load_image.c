...
static int mboot_common_load_part_info(part_dev_t *dev, char *part_name, char *img_name, part_hdr_t *part_hdr, unsigned long long *img_start_addr)
{
...
#ifdef MTK_EMMC_SUPPORT
	start = (u64)part->start_sect * BLK_SIZE;
#else
	start = part->start_sect * BLK_SIZE;
#endif
	...
	while (1) {
		...
		len = dev->read(dev, start + offset, (uchar*)part_hdr, sizeof(part_hdr_t), part->part_id);
#else
		len = dev->read(dev, start + offset, (uchar*)part_hdr, sizeof(part_hdr_t));
#endif
		...
		if (!strncasecmp(part_hdr->info.name, img_name, sizeof(part_hdr->info.name))) {
			dprintf(INFO, "[%s] %s image name not match\n", MODULE_NAME, img_name);
			break;
		}
...
#if defined(MTK_EMMC_SUPPORT) && defined(MTK_NEW_COMBO_EMMC_SUPPORT)
	len = dev->read(dev, start_addr, (uchar*)addr, part_hdr->info.dsize, part->part_id);
#else
	len = dev->read(dev, start_addr, (uchar*)addr, part_hdr->info.dsize);
#endif
