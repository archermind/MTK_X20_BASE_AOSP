...
{
	return (last_lba(EMMC_PART_USER)-((512+PART_MAX_COUNT*sizeof(gpt_entry))/512));
...
int write_primary_gpt(void *data, unsigned sz)
{
	...
	if (sz != (PART_MAX_COUNT*sizeof(gpt_entry)+BLK_SIZE)) {
		err = -1;
		efi_err("[GPT_Update]PGPT size not correct, err(%d), expect: 0x%x read: 0x%x\n", err, (PART_MAX_COUNT*sizeof(gpt_entry)+BLK_SIZE), sz);
		return err;
	}
	if (header->signature != GPT_HEADER_SIGNATURE) {
		err = -1;
	...
	/* Write gpt entries */
	pentries_write_size = (u64)((PART_MAX_COUNT + 3) / 4) * BLK_SIZE;
