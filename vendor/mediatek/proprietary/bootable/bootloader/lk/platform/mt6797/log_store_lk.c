...
void log_store_init(void)
{
	...
	if (sram_header->sig != SRAM_HEADER_SIG) {
		LOG_DEBUG("%s:sram buff header 0x%x is not match: %d!\n", MOD, sram_header, sram_header->sig);
	...
	LOG_DEBUG("%s:sram buff header 0x%x,buff address 0x%x, sig 0x%x, buff_size 0x%x, pl 0x%x : 0x%x, lk size0x%x : 0x%x!\n",
